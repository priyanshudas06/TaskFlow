...
async def create_task(task: TaskCreate, user: dict = Depends(get_current_user)):
    ...
    task_dict["user_id"] = str(user["_id"])
    print(type(user))
...
@router.get("/allTasks/{user_id}", response_model=List[TaskOut])
async def get_tasks(user : dict = Depends(get_current_user)):
    tasks_cursor = task_collection.find({"user_id": str(user["_id"])})
    tasks = []
...
async def get_task(task_id: str, user: dict = Depends(get_current_user)):
    task = await task_collection.find_one({"task_id": task_id, "user_id": str(user["_id"])})
    if not task:
...
async def update_task(task_id: str, updated_task: TaskCreate, user: dict = Depends(get_current_user)):
    existing_task = await task_collection.find_one({"task_id": task_id, "user_id": str(user["_id"])})
    ...
    updated_task_doc = await task_collection.find_one({"task_id": task_id, "user_id": str(user["_id"])})
...
async def delete_task(task_id: str, user: dict = Depends(get_current_user)):
    task = await task_collection.delete_one({"task_id": task_id, "user_id": str(user["_id"])})
    if task.deleted_count == 0:
        raise HTTPException(status_code = 404, detail = "Task not found")
    return {"message":"Task deleted successfully"}

@router.post("/{task_id}/addComment", response_model=CommentCreate)
async def add_comment(task_id: str, comment: CommentCreate, user: dict = Depends(get_current_user)):
    comment_dict = comment.dict()
    comment_dict["task_id"] = task_id
    comment_dict["user_id"] = user["_id"]
    comment_dict["time_stamp"] = datetime.now()
    result = await task_collection.update_one(
        {"task_id": task_id, "user_id" : str(user["_id"])},
    ...
    return CommentCreate(**comment_dict)